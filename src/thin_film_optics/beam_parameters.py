...
def beam_parameters(
    *,
    ...
    Args:
        ...
        angle_inc_degree (ndarrray, range, int, float, optional): Angle of incidence
        in degrees. Defaults to 0.0.
        polarisation (float, optional): polarisation of the wave. Defaults to 0.5.
        wavelength_0 (float, optional): reference wavelength. Defaults to -np.finfo
        (float).eps.
    ...
    wavelength_0_, wavelength_0_index_ = _check_wavelength_0(
        wavelength_,
        wavelength_0,
    )
...
def _check_beam_input(wavelength, angle_inc_degree, polarisation):
    ...
    assert (
        0.0 <= angle_inc_degree.all() <= 90.0
    ), "the angle of incidence should be between 0 and 90 degrees"
